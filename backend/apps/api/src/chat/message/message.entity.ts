...
export class MessageEntity {
  ...
  /**发送者用户ID，0 表示系统消息 */
  @Column({ type: 'int', name: 'sender_id', default: 0, comment: '发送者用户ID' })
  senderId: number;
  ...
  @Column({ type: 'text', name: 'metadata', default: '', comment: '消息元数据' })
  metadata: string;

  /**是否被撤回：true表示已撤回 */
  @Column({ type: 'bool', name: 'is_recalled', default: false, comment: '是否已撤回' })
  isRecalled: boolean;
